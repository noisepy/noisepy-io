...
def test_plot_all_moveout_savefig_no_sdir():
    ...
    with pytest.raises(ValueError, match="sdir"):
        plot_all_moveout(sta_stacks, "Allstack0linear", 0.1, 1.0, "ZZ", 5, savefig=True, sdir=None)


def test_plot_all_moveout_empty():
    # empty list → early return, no error
    plot_all_moveout([], "Allstack0linear", 0.1, 1.0, "ZZ", 5, savefig=False)
...
def test_plot_all_moveout_disp_lag_exceeds_maxlag():
    ...
    with pytest.raises(ValueError, match="lag"):
        plot_all_moveout(sta_stacks, "Allstack0linear", 0.1, 1.0, "ZZ", 5, disp_lag=20.0, savefig=False)
...
def test_plot_all_moveout_no_matching_component():
    ...
    plot_all_moveout(sta_stacks, "Allstack0linear", 0.1, 1.0, "ZR", 5, savefig=False)
...
def test_plot_all_moveout_saves_figure(tmp_path):
    ...
    sta_stacks = [((SRC, REC), [stack])]
    sdir = str(tmp_path)
    plot_all_moveout(sta_stacks, "Allstack0linear", 0.1, 1.0, "ZZ", 10, savefig=True, sdir=sdir)
