...
from .datatypes import CCMethod, ChannelData, ConfigParameters, FreqNorm, RmResp, StackMethod, TimeNorm
    ...
    if rm_resp != RmResp.NO:
        ...
        if rm_resp == RmResp.INV:
            ...
            if not inv[0][0][0].response:
                ...
                raise ValueError(
                    "The response found in the inventory is empty (no stages)! abort!  %s" % st[0]
                )
            else:
                try:
                    ...
                    logger.warning(
                        "Failed to remove response from %s. Returning empty stream. %s" % (st[0], e)
                    )
                    st = []
                    return st
